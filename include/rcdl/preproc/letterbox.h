...
///
/// `yuv` describes the YUV side of any implied colour conversion (levels and
/// matrix, see YuvColorSpace); a bare YuvRange converts to it and means BT.601.
/// Under `Auto`, a combination RGA cannot convert (BT.709 full range, or
/// BT.709 into YUV) takes the CPU path.
LetterboxInfo letterbox(const ImageView& dst, const ImageView& src, std::uint8_t pad = 114,
                        ...
                        YuvColorSpace yuv = {}, PreprocBackend* used = nullptr);
